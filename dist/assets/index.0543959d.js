 ...
 * limitations under the License.
 */
const logger = new Logger$2("@firebase/app");
...
class FirebaseAppImpl {
  constructor(options, config, container) {
    ...
    this.container.addComponent(new Component$3("app", () => this, "PUBLIC"));
  }
...
function registerVersion(libraryKeyOrName, version2, variant) {
  ...
  _registerComponent(new Component$3(`${library}-version`, () => ({ library, version: version2 }), "VERSION"));
}
/**
 * @license
 * Copyright 2021 Google LLC
...
function registerCoreComponents(variant) {
  _registerComponent(new Component$3("platform-logger", (container) => new PlatformLoggerServiceImpl(container), "PRIVATE"));
  _registerComponent(new Component$3("heartbeat", (container) => new HeartbeatServiceImpl(container), "PRIVATE"));
...
function registerAuth(clientPlatform) {
  _registerComponent(new Component$2("auth", (container, { options: deps }) => {
  ...
  _registerComponent(new Component$2("auth-internal", (container) => {
...
function Qo(t2, e) {
  const n = $$1(t2), s = e.snapshotVersion;
  let i = n.qi;
...
async function Ho(t2, e, n) {
  const s = $$1(t2), i = s.qi.get(e), r = n ? "readwrite" : "readwrite-primary";
  try {
...
function fc(t2, e, n) {
  const s = $$1(t2), i = s.queries.get(e);
  if (i)
...
async function vc(t2, e) {
  const n = $$1(t2), s = n.ic.get(e), i = n.rc.get(s.targetId);
  if (i.length > 1)
...
async function Vc(t2, e, n) {
  ...
  try {
    const t3 = await function(t4, e2) {
      const n2 = $$1(t4), s2 = nt.now(), i = e2.reduce((t5, e3) => t5.add(e3.key), Es());
      let r, o;
...
async function xc(t2, e) {
  const n = $$1(t2), s = e.batch.batchId;
  try {
...
!function(t2, e = true) {
  !function(t3) {
    v = t3;
  }(SDK_VERSION), _registerComponent(new Component$1("firestore", (t3, { instanceIdentifier: n, options: s }) => {
 ...
 * limitations under the License.
 */
function noXform_(metadata2, value) {
  return value;
}
class Mapping {
...
function addRef(metadata2, service) {
  function generateRef() {
    const bucket = metadata2["bucket"];
    const path = metadata2["fullPath"];
    const loc = new Location(bucket, path);
    return service._makeStorageReference(loc);
  }
  Object.defineProperty(metadata2, "ref", { get: generateRef });
}
function fromResource(service, resource, mappings) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    metadata2[mapping.local] = mapping.xform(metadata2, resource[mapping.server]);
  }
  addRef(metadata2, service);
  return metadata2;
}
...
function downloadUrlFromResourceString(metadata2, resourceString, host, protocol) {
  const obj = jsonObjectOrNull(resourceString);
  if (obj === null) {
    return null;
  }
...
function toResourceString(metadata2, mappings) {
  const resource = {};
  const len = mappings.length;
  for (let i = 0; i < len; i++) {
    const mapping = mappings[i];
    if (mapping.writable) {
      resource[mapping.server] = metadata2[mapping.local];
    }
  }
...
function metadataHandler(service, mappings) {
  function handler(xhr, text) {
    const metadata2 = fromResourceString(service, text, mappings);
    handlerCheck(metadata2 !== null);
    return metadata2;
  }
  return handler;
}
function downloadUrlHandler(service, mappings) {
  function handler(xhr, text) {
    const metadata2 = fromResourceString(service, text, mappings);
    handlerCheck(metadata2 !== null);
    return downloadUrlFromResourceString(metadata2, text, service.host, service._protocol);
  }
  return handler;
}
...
function determineContentType_(metadata2, blob) {
  return metadata2 && metadata2["contentType"] || blob && blob.type() || "application/octet-stream";
}
function metadataForUpload_(location2, blob, metadata2) {
  const metadataClone = Object.assign({}, metadata2);
...
function multipartUpload(service, location2, mappings, blob, metadata2) {
  ...
  const metadata_ = metadataForUpload_(location2, blob, metadata2);
...
function uploadBytes$1(ref2, data, metadata2) {
  ref2._throwIfRoot("uploadBytes");
  const requestInfo = multipartUpload(ref2.storage, ref2._location, getMappings(), new FbsBlob(data, true), metadata2);
...
function uploadBytes(ref2, data, metadata2) {
  ref2 = getModularInstance(ref2);
  return uploadBytes$1(ref2, data, metadata2);
}
...
const auth = getAuth();
const createUser = (email, passwd) => createUserWithEmailAndPassword(auth, email, passwd);
...
const authState = (user) => onAuthStateChanged(auth, user);
const updateProfileUser = (name2, img) => updateProfile(auth.currentUser, {
  displayName: name2,
  photoURL: img
});
    ...
    <section class="containerAuth__content">
      ...
      <div class= "containerAuth__register-content">
        ...
        <p class="inputs-vacios"></p>
          ...
          <input class="containerAuth__password-form " placeholder="Contrase\xF1a" id="passwordLogin" name="password" type="password">                    
        ...
        <a href="#recuperar" class="containerAuth__register-link" id="reset-password">\xBFOlvidaste tu contrase\xF1a?</a>
      </div>
      <p>\xBFNo tienes cuenta? <a href="#registrar" class="containerAuth__register-link">Registrate</a></p>
    </section>
...
const eventsLogin = () => {
  ...
  $2("#eye").addEventListener("click", () => {
    if ($2(".containerAuth__password-form").type === "password") {
      $2(".containerAuth__password-form").type = "text";
      $2("#icon").classList.remove("fa-eye-slash");
      $2("#icon").classList.add("fa-eye");
    } else if ($2(".containerAuth__password-form").type === "text") {
      $2(".containerAuth__password-form").type = "password";
      $2("#icon").classList.remove("fa-eye");
      $2("#icon").classList.add("fa-eye-slash");
    }
  });
  $2("#formLogin").addEventListener("submit", (e) => {
    e.preventDefault();
    const data = Object.fromEntries(new FormData(e.target));
    if (data.email === "" || data.password === "") {
      $2(".inputs-vacios").innerHTML = "Todos los campos son obligatorios";
      $2("#emailLogin").style.borderColor = "red";
      $2("#passwordLogin").style.borderColor = "red";
      setTimeout(() => {
        $2(".inputs-vacios").innerHTML = "";
      }, 5e3);
    } else {
      loginUser(data.email, data.password).then(() => {
        ...
        if (errorCode.includes("auth/invalid-email")) {
          $2("#emailLogin").style.borderColor = "red";
          $2(".error-mail").innerHTML = "e-mail invalido";
          setTimeout(() => {
            $2(".error-mail").innerHTML = "";
          }, 5e3);
        }
        if (errorCode.includes("auth/wrong-password")) {
          $2("#passwordLogin").style.borderColor = "red";
          $2(".error-password").innerHTML = "Contrase\xF1a invalida";
          setTimeout(() => {
            $2(".error-password").innerHTML = "";
          }, 5e3);
        }
        if (errorCode.includes("auth/user-not-found")) {
          $2("#emailLogin").style.borderColor = "red";
          $2(".error-mail").innerHTML = "Usuario no creado";
          setTimeout(() => {
            $2(".error-mail").innerHTML = "";
          }, 5e3);
        }
  ...
  $2("#loginGoogle").addEventListener("click", () => {
    const promise = loginGoogle();
    promise.then(() => {
      window.location.hash = "#timeline";
    });
  });
  $2("#reset-password").addEventListener("click", () => {
...
const welcome = () => {
  ...
  <section class="container__content">
    ...
    <p class="container__welcome-content">Con esta App podr\xE1s compartir tus comida favorita, hacer review, buscar y localizar platillos exquisitos en tu pa\xEDs</p>
    <div class="container__buttons-content">
      <a href="#login" class="container__login-buttons" id="login">Iniciar Sesi\xF3n</a>
    ...
    <section class="containerAuth__content">
      ...
      <div class= "containerAuth__register-content">
        <form class="containerAuth__form-register" id="formRegister">
          ...
          <input class="containerAuth__password-form " placeholder="Contrase\xF1a" id="password" name="password" type="password">
          <p class="error-password"></p> 
          <input type="text" class="containerAuth__country-form password-eye" placeholder="Pa\xEDs" id="country" name="country"><br>          
...
const eventsRegistro = () => {
  const $2 = (selector) => document.querySelector(selector);
  $2("#formRegister").addEventListener("submit", (e) => {
    e.preventDefault();
    const data = Object.fromEntries(new FormData(e.target));
    if (data.email === "" || data.password === "") {
      $2(".inputs-vacios").innerHTML = "Todos los campos son obligatorios";
      $2("#name").style.borderColor = "red";
      $2("#country").style.borderColor = "red";
      $2("#email").style.borderColor = "red";
      $2("#password").style.borderColor = "red";
      setTimeout(() => {
        $2(".inputs-vacios").innerHTML = "";
      }, 5e3);
    } else {
      createUser(data.email, data.password).then(() => {
        ...
        if (errorCode === "auth/email-already-in-use") {
          $2(".error-mail").innerHTML = "Este email ya se encuentra en uso";
          setTimeout(() => {
            $2(".error-mail").innerHTML = "";
          }, 5e3);
        }
        if (errorCode === "auth/weak-password") {
          $2(".error-password").innerHTML = "La contrase\xF1a debe tener m\xEDnimo 6 c\xE1racteres";
          setTimeout(() => {
            $2(".error-password").innerHTML = "";
          }, 5e3);
        }
  ...
  $2("#registerGoogle").addEventListener("click", (e) => {
    e.preventDefault();
    loginGoogle().then(() => {
      window.location.hash = "#timeline";
    });
  });
  $2("#eye-registro").addEventListener("click", () => {
    if ($2(".containerAuth__password-form").type === "password") {
      $2(".containerAuth__password-form").type = "text";
      $2("#icon").classList.remove("fa-eye-slash");
      $2("#icon").classList.add("fa-eye");
...
const addPost = (post) => Ql(Ta(db, "posts"), post);
const onGetPost = (callback) => jl(Ta(db, "posts"), callback);
...
const storageRef = (path, root) => ref(storage, `${root}/${path.name}`);
const uploadTask = (storageReference, file) => uploadBytes(storageReference, file, metadata);
const getDownloadIMG = (path) => getDownloadURL(ref(storage, path));
const modalPost = () => {
  const modal = `<section class="modal-container">
    <div class="title__modal">
      <h1 class="titulo">Crear Publicaci\xF3n</h1>
    ...
    <form class="create-publication" id="formPublication">
      ...
      <input type="text" required name="pais" class="input-plubication" id="country-product" placeholder="Pa\xEDs"></input>
      <input type="text" required name="ubicacion" class="input-plubication" id="here-product" placeholder="Lugar/Ubicaci\xF3n"></input>
      <input type="text" required name="puntos" class="input-plubication" id="point-product" placeholder="Puntuaci\xF3n 0/10"></input>
...
const eventsModalPost = () => {
  const $2 = (selector) => document.querySelector(selector);
  $2("#btn-cerrar-modal").addEventListener("click", () => {
    $2(".modal").innerHTML = "";
  });
  $2("#input-file-photo").addEventListener("change", () => {
    const path = $2("#input-file-photo").files[0];
    $2("#formPublication").addEventListener("submit", (e) => {
      e.preventDefault();
      const data = Object.fromEntries(new FormData(e.target));
      ...
      data.likes = [];
      uploadTask(storageRef(path, "images"), path).then((res) => {
        getDownloadIMG(res.ref.fullPath).then((url) => {
...
const eventsEditarPost = (id2) => {
  const $2 = (selector) => document.querySelector(selector);
  $2(".titulo").innerHTML = "Editar Publicacion";
  getPost(id2).then((res) => {
    $2("#name-product").value = res.data().producto;
    $2("#country-product").value = res.data().pais;
    $2("#here-product").value = res.data().ubicacion;
    $2("#point-product").value = res.data().puntos;
  });
  $2("#input-file-photo").addEventListener("change", () => {
    $2("#formPublication").addEventListener("submit", (e) => {
      const data = Object.fromEntries(new FormData(e.target));
...
const Post = (post, id2, idUserDB, imgProfile) => {
  const page = `<section class="container__post">
    ...
    <div class="botones">
      <div class="btn-like">
        <i class="fa-regular fa-heart ${post.likes.includes(currentUserInfo().email) ? "true" : "false"}" id="${id2}">
        </i>
      ...
      ${idUserDB === currentUserInfo().uid ? `<i class="fa-regular fa-pen-to-square ${idUserDB}" id="${id2}"></i><i class="fa-solid fa-trash-can ${idUserDB}" id="${id2}"></i>` : ""}
      </div>
    </div>
  </section>`;
  return page;
...
const eventsPost = () => {
  ...
  btnsEliminar.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      const id2 = e.target.id;
      if (confirm("\xBFEst\xE1s seguro que deseas eliminar tu post?") === true) {
        deletePost(id2);
      }
    });
  });
  ...
  btnsEditar.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      const id2 = e.target.id;
      $2(".modal").innerHTML = modalPost();
      eventsEditarPost(id2);
    });
  });
};
const timeline = () => {
...
const eventsTimeLine = () => {
  ...
  onGetPost((res) => {
    ...
    arr.sort((a, b2) => b2.fecha - a.fecha);
    arr.forEach((post) => {
      posts += Post(
        post,
        post.id,
    ...
    $2(".posts").innerHTML = "";
    $2(".posts").insertAdjacentHTML("beforeend", posts);
    eventsPost();
  });
  authState((user) => {
    if (user !== null) {
      const fotoUsuario = user.photoURL;
      $2("#foto-perfil-post").src = fotoUsuario;
      $2("#foto-perfil-post").referrerpolicy = "no-referrer";
      $2("#foto-perfil").src = fotoUsuario;
      $2("#foto-perfil").referrerpolicy = "no-referrer";
    }
  });
  $2("#btn-post").addEventListener("click", () => {
    $2(".modal").innerHTML = modalPost();
    eventsModalPost();
  });
  $2("#foto-perfil").addEventListener("click", () => {
    window.location.hash = "#perfil";
  });
  $2(".fa-house").addEventListener("click", () => {
    window.scrollTo(0, 0);
  });
  $2("#cerrar-sesion").addEventListener("click", (e) => {
    e.preventDefault();
    signOutUser().then(() => {
...
const resetPassword = () => {
  const pageResetPassword = `<section class="containerAuth container">    
    <section class="containerAuth__content">
      <h1 class="containerAuth__title-content">Recupera tu contrase\xF1a</h1>
      <div class= "containerAuth__register-content">
...
const eventsResetPassword = () => {
  const $2 = (selector) => document.querySelector(selector);
  $2("#formResetPassword").addEventListener("submit", (e) => {
    e.preventDefault();
    const data = Object.fromEntries(new FormData(e.target));
    resetPasswordEmail(data.email).then(() => {
      alert("email enviado");
      window.location.hash = "#login";
    });
  });
...
const eventsPerfil = () => {
  ...
  authState((user) => {
    if (user !== null) {
      $2(".photo-user-profile").src = user.photoURL;
      $2(".input-name-user").value = user.displayName;
      $2("#foto-perfil").src = user.photoURL;
      $2(".name-user").textContent = user.displayName;
    }
  });
  $2(".fa-house").addEventListener("click", () => {
    window.location.hash = "#timeline";
  });
  $2(".update-profile").addEventListener("click", (e) => {
    e.preventDefault();
    const name2 = $2(".input-name-user").value;
    updateProfileUser(name2);
    $2(".name-user").textContent = name2;
  });
  $2("#input-file-photo").addEventListener("change", () => {
    const name2 = $2(".input-name-user").value;
    const path = $2("#input-file-photo").files[0];
    uploadTask(storageRef(path, "profiles"), path).then((res) => {
      getDownloadIMG(res.ref.fullPath).then((url) => {
        $2(".photo-user-profile").src = url;
        updateProfileUser(name2, url);
        $2(".name-user").textContent = name2;
      });
    });
  });
  $2("#btn-cerrar-profile").addEventListener("click", () => {
    window.location.hash = "#timeline";
  });
};
const Router = () => {
